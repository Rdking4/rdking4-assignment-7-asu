...
class SLThread extends Thread {
    ...
    public void run() {
        ...
        try {
            serverSocket = new ServerSocket(IStart.DEFAULT_PORT);
            serverSocket.accept();
            IStart.app.show();
            serverSocket.close();
            new SLThread().start();
            ...
            System.err.println("Port:"+IStart.DEFAULT_PORT);
            e.printStackTrace();
            new ExceptionDialog(e, "Cannot create a socket connection on localhost:"+IStart.DEFAULT_PORT,
            "Make sure that other software does not use the port "+IStart.DEFAULT_PORT+" and examine your security settings.");
        }
    }
}
